...
public class NutritionDecripter {
	...
	public Map<String, Float> getIngredientFacts(String measurement, String type, float amount, String cal, String fatt, String carb, String prot){
		...
		info.put(TAG_CALORIES, (float) Math.round(amount* weight*(calories/100)*10.0f)/10.0f);
		info.put(TAG_FAT, (float) Math.round(amount* weight*(fat/100)*10.0f)/10.0f);
		info.put(TAG_CARBS, (float) Math.round(amount* weight*(carbs/100)*10.0f)/10.0f);
		info.put(TAG_PROTEIN, (float) Math.round(amount* weight*(protein/100)*10.0f)/10.0f);
